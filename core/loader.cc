...
void Be::Loader::Compile(V8_ARGS) {
  ...
  while (1) {
    ...
    if (ret == -1) {
      return args.GetReturnValue().Set(
          newStringToLcal(isolate, "read file error"));
    }
    if (ret == 0) {
      break;
    }
  ...
  ScriptCompiler::Source script_source(
      newStringToLcal(isolate, content.c_str()));
  Local<String> params[] = {
      newStringToLcal(isolate, "require"),
      newStringToLcal(isolate, "exports"),
      newStringToLcal(isolate, "module"),
  };
  MaybeLocal<Function> fun = ScriptCompiler::CompileFunction(
      context, &script_source, 3, params, 0, nullptr);
  if (fun.IsEmpty()) {
...
void Be::Loader::Init(Isolate* isolate, Local<Object> target) {
  // 创建 loader V8对象模板(对象模板中的属性都会添加进通过 loader
  // 对象模板创建的对象实例中)
